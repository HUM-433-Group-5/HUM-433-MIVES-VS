...
class Ui_MainWindow(QMainWindow):
    def setupUi(self, MainWindow):
        ...
        save_button = QtWidgets.QPushButton(self.centralwidget)
        save_button.setGeometry(QtCore.QRect(810, self.buttony, 200, 30))
        self.buttony = self.buttony + 30
        save_button.setObjectName("Save tree")
        save_button.setText("Save tree")
        save_button.clicked.connect(self.save_tree)

        load_button = QtWidgets.QPushButton(self.centralwidget)
        load_button.setGeometry(QtCore.QRect(810, self.buttony, 200, 30))
        self.buttony = self.buttony + 30
        load_button.setObjectName("Load tree")
        load_button.setText("Load tree")
        load_button.clicked.connect(self.load_tree)
        
# Start for Database
        self.buttony = self.buttony + 30
        
        self.material_list = ['concrete','wood','metal']
        self.database_cb = QtWidgets.QCheckBox("Use Database",self.centralwidget)
        self.database_cb.setGeometry(QtCore.QRect(810, self.buttony, 200, 30))
        self.buttony = self.buttony + 30
        self.database_cb.setObjectName("Use Database")
        ...
        for i in range(num_of_materials):
            ...
            combobox.setGeometry(QtCore.QRect(810, self.buttony, 200, 30))
            self.buttony = self.buttony + 30
...
# End for Database
    ...
    def save_tree(self):
        Dialog = QtWidgets.QDialog()
        ui = Ui_Dialog()
        ui.setupUi(Dialog, True, "saved_tree.json")
        Dialog.show()
        rsp = Dialog.exec_()
        if rsp == QtWidgets.QDialog.Accepted:
            file_name = ui.branch_name.text()
            if not file_name.endswith('.json'):
                file_name += '.json'

            json_data = {
                'tree': self.t.write(format=1),
                'weights': self.weights,
                'variants': self.variants,
                'indicator_dict': self.indicator_dictionnary
            }
    ...
    def load_tree(self):
        ...
        if rsp == QtWidgets.QDialog.Accepted:
            ...
            if exists(file_name):
                ...
                self.t = Tree(json_data['tree'], format=1)

                self.weights = json_data['weights']
                self.variants = json_data['variants']
                self.indicator_dictionnary = json_data['indicator_dict']
                self.name_faces = {}
                self.weight_faces = {}
                ...
                message_box.setText("Cannot read save file.")
...
# Start for Database
    def set_use_database(self):
        if self.database_cb.isChecked():
            ...
            path = join(current_dir, 'database', 'database.csv')
    ...
    def call_msgbox(self, title, msg, size_x = 0, size_y = 0):
        dialog = QDialog()
